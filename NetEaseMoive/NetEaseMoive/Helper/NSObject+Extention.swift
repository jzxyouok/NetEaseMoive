...
extension NSObject{
    func setValuesForProperies(properies:NSArray,values:NSDictionary){
        for var i = 0;i < properies.count;i++ {
            let property = properies[i] as! SEPropery
    ...
    class func properies(replaceDic:NSDictionary?) -> NSArray{
        ...
        for var i = 0;i < Int(outCount);i++ {
            ...
            let propery = SEPropery(propery: property)
...
private class SEPropery {
    ...
    func setup(){
        ...
        self.properyType = SEType(code: types)
    }
}
private class SEType {
    var code:NSString
